...
public class SnapperTransactionalFineAccountGrain : TransactionExecutionGrain, ISnapperTransactionalAccountGrain
{
    readonly string grainName = typeof(SnapperTransactionalFineAccountGrain).FullName;

    public SnapperTransactionalFineAccountGrain(ISnapperClusterCache snapperClusterCache, ISnapperReplicaCache snapperReplicaCache, IScheduleManager scheduleManager, ISnapperLoggingHelper log)
        : base(typeof(SnapperTransactionalFineAccountGrain).FullName, snapperClusterCache, snapperReplicaCache, scheduleManager, log) { }
    ...
    public async Task<object?> Init(TransactionContext cxt, object? obj = null)
    {
        ...
        var keysToAccess = new Dictionary<ISnapperKey, AccessMode>();
        for (var i = 0; i < numAccounts; i++)
        {
            var userID = SmallBankIdMapping.GetUserID(myID.grainID, i);
            keysToAccess.Add(userID, AccessMode.ReadWrite);
        }

        (var dictionaryState, _) = await GetFineState(cxt, keysToAccess);

        foreach (var item in keysToAccess)
            dictionaryState.Put(item.Key, new AccountInfo(1000000));
    ...
    public async Task<object?> MultiTransfer(TransactionContext cxt, object? obj = null)
    {
        ...
        (var dictionaryState, _) = await GetFineState(cxt, keyAccessMode);
