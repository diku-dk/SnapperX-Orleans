...
public class SnapperTransactionalKeyValueShipmentActor : TransactionExecutionGrain, IShipmentActor
{
    int sellerID;
    ...
    public SnapperTransactionalKeyValueShipmentActor(ISnapperClusterCache snapperClusterCache, ISnapperReplicaCache snapperReplicaCache, IScheduleManager scheduleManager, ISnapperLoggingHelper log)
        : base(typeof(SnapperTransactionalKeyValueShipmentActor).FullName, snapperClusterCache, snapperReplicaCache, scheduleManager, log) {}
    ...
    public async Task<object?> Init(TransactionContext cxt, object? obj = null)
    {
        ...
        var sellerActorID = new GrainID(Helper.ConvertIntToGuid(sellerID, baseCityID), typeof(SnapperTransactionalKeyValueSellerActor).FullName);

        nextPackageIDKey = new GeneralStringKey("nextPackageID");
        (var dictionaryState, var _) = await GetKeyValueState(AccessMode.ReadWrite, cxt, new HashSet<ISnapperKey>{ nextPackageIDKey, seller });
    ...
    public async Task<object?> NewPackage(TransactionContext cxt, object? obj = null)
    {
        ...
        (var dictionaryState, var listState) = await GetKeyValueState(AccessMode.ReadWrite, cxt, new HashSet<ISnapperKey>{ nextPackageIDKey, seller });
        ...
        //(dictionaryState, _) = await GetKeyValueState(AccessMode.ReadWrite, cxt, new HashSet<ISnapperKey> { packageID });
