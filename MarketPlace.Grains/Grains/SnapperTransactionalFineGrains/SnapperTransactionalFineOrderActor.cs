...
public class SnapperTransactionalFineOrderActor : TransactionExecutionGrain, IOrderActor
{
    int customerID;
    ...
    public SnapperTransactionalFineOrderActor(ISnapperClusterCache snapperClusterCache, ISnapperReplicaCache snapperReplicaCache, IScheduleManager scheduleManager, ISnapperLoggingHelper log)
        : base(typeof(SnapperTransactionalFineOrderActor).FullName, snapperClusterCache, snapperReplicaCache, scheduleManager, log) { }
    ...
    public async Task<object?> Init(TransactionContext cxt, object? obj = null)
    {
        ...
        (var dictionaryState, var _) = await GetFineState(cxt, new Dictionary<ISnapperKey, AccessMode> { { nextOrderIDKey, AccessMode.ReadWrite } });
    ...
    public async Task<object?> NewOrder(TransactionContext cxt, object? obj = null)
    {
        ...
        foreach (var item in itemsPerSeller)
        {
            ...
            var stockActorID = new GrainID(stockActorGuid, typeof(SnapperTransactionalFineStockActor).FullName);

            method = typeof(SnapperTransactionalFineStockActor).GetMethod("ReduceStock");
        ...
        var paymentActorID = new GrainID(paymentActorGuid, typeof(SnapperTransactionalFinePaymentActor).FullName);

        method = typeof(SnapperTransactionalFinePaymentActor).GetMethod("ProcessPayment");
        Debug.Assert(method != null);
    ...
    async Task CreateOrder(TransactionContext cxt, PaymentInfo paymentInfo, Dictionary<SellerID, List<OrderItem>> orderItemsPerSeller, int deliveryCityID)
    {
        ...
        (var dictionaryState, var listState) = await GetFineState(cxt, new Dictionary<ISnapperKey, AccessMode> { { nextOrderIDKey, AccessMode.ReadWrite } });
        ...
        foreach (var item in orderItemsPerSeller)
        {
            ...
            var shipmentActorID = new GrainID(shipmentActorGuid, typeof(SnapperTransactionalFineShipmentActor).FullName);
            var method = typeof(SnapperTransactionalFineShipmentActor).GetMethod("NewPackage");
        ...
        foreach (var t in tasks)
        {
            ...
            //(dictionaryState, _) = await GetFineState(cxt, new Dictionary<ISnapperKey, AccessMode> { { referenceInfo.key2, AccessMode.ReadWrite } });
        ...
        //(dictionaryState, _) = await GetFineState(cxt, new Dictionary<ISnapperKey, AccessMode> { { orderID, AccessMode.ReadWrite } });
